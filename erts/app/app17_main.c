...
// number of cars leaving the intersection at each cycle of the traffic light
#define MAX_TRAFFIC_FLOW	3		// during normal operation
#define MIN_TRAFFIC_FLOW	1		// in case of car accident
int traffic_flow = MAX_TRAFFIC_FLOW;
...
// mutex on the tow truck
 pthread_mutex_t  mutexOnTowTruck  = PTHREAD_MUTEX_INITIALIZER;

// condition variable on the tow truck
pthread_cond_t condOnTowTruck =  PTHREAD_COND_INITIALIZER; 

// shared variable for the tow truck
bool tow_truck_operation= false;

// state variable for the car_accident
bool car_accident = false;
...
void print_queues() {
	if(ambulance_arriving) {
		...
		if(direction_green == 1)
			printf("-----  GO N-S  flow=%d  ----\n", traffic_flow);
		else
			printf("-----  GO E-W  flow=%d  ----\n", traffic_flow);
	}
	printf("    N-S       E-W ");
	printf("\n     %d         %d\n\n", queue_NS, queue_EW);
}
...
void ActivityTrafficLight(int duration) {
	...
	/*
	 * check if there is a car accident
	 */ 
	if(car_accident == false) {
		traffic_flow = MAX_TRAFFIC_FLOW;
		
		// simulate a random car accident
		int r_num_accident = rand() % 100 + 1;	// random number between 1 and 100
		if(r_num_accident > 90) {
				printf("\n$$$$$$$$$$$$$$ ACCIDENT $$$$$$$$$\n");
				// there is a car accident
				car_accident = true;

				// reduce the traffic flow
				traffic_flow = MIN_TRAFFIC_FLOW;
		
				// call the tow truck
				pthread_mutex_lock( &mutexOnTowTruck );
				pthread_cond_signal(&condOnTowTruck);//NOTIFY
				pthread_mutex_unlock( &mutexOnTowTruck );    	
		}
	}
	else {
		// ckeck if the car accident has been resolved
		pthread_mutex_lock( &mutexOnTowTruck );
			if(tow_truck_operation == false) {
				printf("\n$$$$$$$$$$$$$$ ACCIDENT RESOLVED $$$$$$$$$\n");	
				car_accident = false;
				traffic_flow = MAX_TRAFFIC_FLOW;
			}
	...
	// the traffic light is green in the N-S or E-W direction
	pthread_mutex_lock( &mutexOnQueue );
	if(direction_green == 1) {	// greed along the NS direction
		if(queue_NS > traffic_flow)  
			queue_NS -= traffic_flow;
		else
			queue_NS = 0;
	}
	else	 {
		if(queue_EW > traffic_flow)	// green along the EW direction
		 	queue_EW -= traffic_flow;
		else
			queue_EW = 0;
	}
	print_queues();
...
/*
 * Function that implements the intervention of the tow truck
 * the input parameter "duration" is used to specify the duration of the intervention
 */
void* ActivityTowTruck(void* par) {
	int duration = *(int*) par;
	while(1) {
		pthread_mutex_lock( &mutexOnTowTruck );
		printf("\n*** *** *** TOW TRUCK  WAITING *** *** ***\n");
		pthread_cond_wait(&condOnTowTruck, &mutexOnTowTruck);
		tow_truck_operation = true;
		pthread_mutex_unlock( &mutexOnTowTruck );    	
...
int main(int argc, char* argv[]) {
	...
	/* Create the aperiodic thread for the TowTruck activity*/
	int duration = 15;
    ret_err = pthread_create( &thread1, NULL, ActivityTowTruck, (void*) &duration);
	...
	sprintf(activity_2.name,	"ActivityTrafficLight ");
	activity_2.function = ActivityTrafficLight;
	...
	sprintf(activity_3.name,	"ActivityCarsArriving");
	activity_3.function = ActivityCarsArriving;
	activity_3.period = 600;
	activity_3.parameter = 2;
	activity_3.print = false;
    ret_err = pthread_create( &thread1, NULL, PeriodicTask, (void*) &activity_3);
	...
	sprintf(activity_4.name,	"ActivityAmbulanceArriving");
	activity_4.function = ActivityAmbulanceArriving;
	activity_4.period = 3000;
	activity_4.parameter = 50;
	activity_4.print = false;
    ret_err = pthread_create( &thread2, NULL, PeriodicTask, (void*) &activity_4);
    handle_error(ret_err, "Error in creating PeriodicTask 4");
