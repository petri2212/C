...
t_bug* bug_new(t_matrix* field, int l) {
	if(field->type != 'c') {
		printf("ERROR: Wrong data type for the field matrix!");
		return NULL;
	}
...
void bug_dump(t_bug* bug) {
	((char*)bug->field->data)[bug->body[0]] = 'o';
	for(int i=1; i < bug->length; i++)
		((char*)bug->field->data)[bug->body[i]] = 'x';
}
